...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, x: Tensor, attention_mask: Tensor) -> Tensor:
        number_of_nodes, feature_dim = x.size()
        assert feature_dim == self.in_features

        # project node features
        query = self.W(x).view(number_of_nodes, self.n_heads, self.out_features)
        key = self.W(x).view(number_of_nodes, self.n_heads, self.out_features)

        query = query.repeat(number_of_nodes, 1, 1)
        key = key.repeat_interleave(number_of_nodes, dim=0)

        g_sum = query + key
        g_sum = g_sum.view(number_of_nodes, number_of_nodes, self.n_heads, self.out_features)
        ...
        attn_res = torch.einsum('ijh,jhf->ihf', a, key)
...
class AGG(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.node_feature_dim = input_dim + time_embedding_dim
        self.query_dim = time_embedding_dim
        self.time_embed = Time2Vec(time_embedding_dim)
        self.self_attention_graph = GraphAttentionLayer(
