...
class AsynchronousGraphGenerator(nn.Module):
    ...
    def __init__(
        ...
        if spatial_embedding_dim == 0:
            ...
            self.spatial_embed = nn.Embedding(
                num_spatial_components, spatial_embedding_dim
            )
        ...
        for key, value in state_dict.items():
            ...
            if stem[0] in self.valid_state_names:
                ...
                if stem[1] in self.valid_state_names:
                    ...
                    print(f"Ignored {key=}")
        super(AsynchronousGraphGenerator, self).load_state_dict(new_state_dict, strict)
    ...
    def forward(
        ...
        if graph.category_index is not None and graph.category_index is not None:
        ...
        if self.spatial_embed is not None and graph.target.spatial_index is not None:
            target_spatial_encode = self.spatial_embed(
                graph.target.spatial_index.to(device)
            )
            query_list.append(target_spatial_encode)
        if (
            self.query_includes_type
            and isinstance(
                graph.target.type_index, (torch.LongTensor, torch.FloatTensor)
            )
            and self.type_embed is not None
        ):
        ...
        if graph.key_padding_mask is not None:
